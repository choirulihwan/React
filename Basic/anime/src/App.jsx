...
//statefull komponen
function Search({children, onFilteredAnime}) {
  
  return (
    <div className="search-container">
      <input className="search" type="text" placeholder="Search anime..." onChange={onFilteredAnime} />
      {children}
    </div>
  );
}
...
function AnimeDetail({selectedAnime}) {
  ...
  if ((selectedAnime.length == undefined) || (selectedAnime.length > 0)) {
    return (
...
//structural component => berfungsi utk menggabungkan dan tidak reusable
export default function App() {
  ...
  function handleFilteredAnime(e) {    
    const filteredAnime = animesData.filter(anime => anime.title.toLowerCase().includes(e.target.value.toLocaleLowerCase()));
    if (filteredAnime.length >0) {
      setAnimes(filteredAnime);
      setSelectedAnime(filteredAnime[0]);
    } else {
      setAnimes([]);
      setSelectedAnime([])
    }    
  ...
  return (
    <>
      <Navbar>
        <Search onFilteredAnime={handleFilteredAnime}>
          <NumResult animes={animes} />
        </Search>
      </Navbar>

      {/* container component untuk mengatasi component drilling */}
      <Main>
        <Box>
